...
# seems to be similar issue as ui import
import nslr_hmm


# %%
def detect_events_hmm_nosmooth(etsamples, etevents):
    etsamples, etevents = detect_events_hmm(etsamples, etevents, smoothpursuit=False)
    return etsamples, etevents


def detect_events_hmm(etsamples, etevents, smoothpursuit=True):
...
def sampletype_to_event(etsamples, eventtype):
    ...
    events = pd.concat(
        [events, etsamples.loc[etsamples['tmp'] == 1, 'gx'].astype(float).reset_index(drop=True).rename('start_gx')],
        axis=1)
    events = pd.concat(
        [events, etsamples.loc[etsamples['tmp'] == 1, 'gy'].astype(float).reset_index(drop=True).rename('start_gy')],
        axis=1)
    events = pd.concat(
        [events, etsamples.loc[etsamples['tmp'] == -1, 'gx'].astype(float).reset_index(drop=True).rename('end_gx')],
        axis=1)
    events = pd.concat(
        [events, etsamples.loc[etsamples['tmp'] == -1, 'gy'].astype(float).reset_index(drop=True).rename('end_gy')],
        axis=1)
    ...
    events['amplitude'] = events.apply(
        lambda localrow: make_df.calc_3d_angle_points(localrow.start_gx, localrow.start_gy, localrow.end_gx,
                                                      localrow.end_gy), axis=1)
    ...
    for ix, row in events.iterrows():
        ...
        eventdf = pd.DataFrame(
            {'x0': etsamples.loc[ix_samples].iloc[:-1].gx.values, 'y0': etsamples.loc[ix_samples].iloc[:-1].gy.values,
             'x1': etsamples.loc[ix_samples].iloc[1:].gx.values, 'y1': etsamples.loc[ix_samples].iloc[1:].gy.values})
        thetas = eventdf.apply(
            lambda localrow: make_df.calc_3d_angle_points(localrow.x0, localrow.y0, localrow.x1, localrow.y1), axis=1)
    ...
    return (events)
# if 1 == 0: subject = 'VP4' etsamples, etmsgs, etevents = preprocess.preprocess_et('el',subject,
# datapath='/home/behinger/etcomp/local/data/',load=False,save=False,outputprefix='hmm_',eventfunctions=(make_blinks,
# detect_events_hmm))
