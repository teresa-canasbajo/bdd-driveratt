    ...
    for subject in subjectnames:
        for et in ets:
            ...
            if not data:
                ...
                etsamples, etmsgs, etevents = (
                d.query("eyetracker=='" + et + "'&subject=='" + subject + "'").drop(["eyetracker", "subject"], axis=1)
                for d in data)
            ...
            if condition in ['LARGE_GRID', 'LARGE_and_SMALL_GRID', 'SMOOTHPURSUIT', 'MICROSACC', 'SHAKE', 'TILT']:
                ...
                merged_events = helper.add_msg_to_event(etevents, etmsgs.query(
                    "condition=='BLINK'&(exp_event=='stop'|exp_event=='start')"), timefield='start_time',
                                                        direction='backward')
                ...
                merged_events = helper.add_msg_to_event(etevents, etmsgs.query('condition=="FREEVIEW"'),
                                                        timefield='start_time', direction='forward')
            ...
            condition_df.loc[:,
            'eyetracker'] = et  # behinger added this to keep same as etsamples/etmsgs/etevents. 'et' is later renamed
