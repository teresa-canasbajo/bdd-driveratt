...
def make_large_grid_df(merged_events):
    ...
    large_grid_events = merged_events.query('condition == "GRID"').loc[:,
                        ['sd', 'type', 'end_time', 'mean_gx', 'duration', 'start_time', 'rms', 'mean_gy', 'block',
                         'condition', 'element', 'exp_event', 'grid_size', 'msg_time', 'posx', 'posy']]
    # use the last exp_event fixation as element 50
    stopevents = large_grid_events.query('exp_event=="stop"').assign(element=50., grid_size=49., posx=0, posy=0,
                                                                     exp_event='element')
...
def make_condition(merged_events, condition=None):
    ...
    if condition == "LARGE_GRID":
        ...
        small_grid_before_events = merged_events.query('condition == "SMALLGRID_BEFORE"').loc[:,
                                   ['sd', 'type', 'end_time', 'mean_gx', 'duration', 'start_time', 'rms', 'mean_gy',
                                    'block', 'condition', 'element', 'exp_event', 'grid_size', 'msg_time', 'posx',
                                    'posy']]

        # only small grid after condition
        small_grid_after_events = merged_events.query('condition == "SMALLGRID_AFTER"').loc[:,
                                  ['sd', 'type', 'end_time', 'mean_gx', 'duration', 'start_time', 'rms', 'mean_gy',
                                   'block', 'condition', 'element', 'exp_event', 'grid_size', 'msg_time', 'posx',
                                   'posy']]
        ...
        out_df = merged_events.query('condition == "SHAKE"').loc[:,
                 ['sd', 'type', 'end_time', 'mean_gx', 'duration', 'start_time', 'rms', 'mean_gy', 'block', 'condition',
                  'element', 'exp_event', 'grid_size', 'msg_time', 'shake_x', 'shake_y']]
...
def make_all_elements_grid_df(merged_events):
    ...
    # only small grid before condition
    small_grid_before_events = merged_events.query('condition == "SMALLGRID_BEFORE"').loc[:,
                               ['type', 'end_time', 'mean_gx', 'duration', 'start_time', 'rms', 'mean_gy', 'block',
                                'condition', 'element', 'exp_event', 'grid_size', 'msg_time', 'posx', 'sd', 'posy']]

    # only small grid after condition
    small_grid_after_events = merged_events.query('condition == "SMALLGRID_AFTER"').loc[:,
                              ['type', 'end_time', 'mean_gx', 'duration', 'start_time', 'rms', 'mean_gy', 'block',
                               'condition', 'element', 'exp_event', 'grid_size', 'msg_time', 'posx', 'posy', 'sd']]

    # take only elements that are in small and large grid
    all_elements_df = pd.concat([large_grid_events, small_grid_before_events, small_grid_after_events],
                                ignore_index=True)
...
def make_freeview_df(merged_freeview_events):
    ...
    all_freeview_events = merged_freeview_events.loc[:,
                          ['msg_time', 'condition', 'exp_event', 'block', 'trial', 'pic_id', 'type', 'start_time',
                           'end_time', 'duration', 'mean_gx', 'sd', 'mean_gy', 'rms']]
