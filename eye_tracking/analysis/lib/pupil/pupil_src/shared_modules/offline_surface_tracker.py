...
from eye_tracking.analysis.lib.pupil.pupil_src.shared_modules import gl_utils
from eye_tracking.analysis.lib.pupil.pupil_src.shared_modules.methods import normalize
from eye_tracking.analysis.lib.pupil.pupil_src.shared_modules.file_methods import Persistent_Dict
from eye_tracking.analysis.lib.pupil.pupil_src.shared_modules.cache_list import Cache_List
from glfw import *
from pyglui import ui
from pyglui.cygl.utils import *
from pyglui.pyfontstash import fontstash

from eye_tracking.analysis.lib.pupil.pupil_src.shared_modules.plugin import Analysis_Plugin_Base
# logging
import logging

logger = logging.getLogger(__name__)

from eye_tracking.analysis.lib.pupil.pupil_src.shared_modules.surface_tracker import Surface_Tracker
from eye_tracking.analysis.lib.pupil.pupil_src.shared_modules.square_marker_detect import draw_markers, m_marker_to_screen
...
class Offline_Surface_Tracker(Surface_Tracker, Analysis_Plugin_Base):
    ...
    def load_marker_cache(self):
        ...
        if cache is None:
            ...
            self.invert_image = self.persistent_cache.get('inverted_markers', False)
            logger.debug("Loaded marker cache {} / {} frames had been searched before".format(
    ...
    def load_surface_definitions_from_file(self):
        ...
        if self.surface_definitions.get('offline_square_marker_surfaces', []) != []:
            ...
            self.surfaces = [Offline_Reference_Surface(self.g_pool, saved_definition=d) for d in
                             self.surface_definitions.get('offline_square_marker_surfaces', [])]
    ...
    def init_ui(self):
        ...
        self.add_button = ui.Thumb('add_surface', setter=lambda x: self.add_surface(), getter=lambda: False, label='A',
                                   hotkey='a')
    ...
    def update_gui_markers(self):
        ...
        self.menu.append(
            ui.Slider('min_marker_perimeter', self, min=20, max=500, step=1, setter=set_min_marker_perimeter))
    ...
    def recent_events(self, events):
        ...
        if self.mode == "Show marker IDs":
            ...
            if self.edit_surf_verts:
                window = eye_tracking.analysis.lib.pupil.pupil_src.shared_modules.glfwGetCurrentContext()
                pos = eye_tracking.analysis.lib.pupil.pupil_src.shared_modules.glfwGetCursorPos(window)
    ...
    def init_marker_cacher(self):
        ...
        from eye_tracking.analysis.lib.pupil.pupil_src.shared_modules.marker_detector_cacher import fill_cache
    ...
    def save_surface_statsics_to_file(self, export_range, export_dir):
        ...
        with open(os.path.join(metrics_dir, 'surface_gaze_distribution.csv'), 'w', encoding='utf-8',
                  newline='') as csvfile:
            csv_writer = csv.writer(csvfile, delimiter=',')
        ...
        with open(os.path.join(metrics_dir, 'surface_events.csv'), 'w', encoding='utf-8', newline='') as csvfile:
            ...
            for e in events:
                csv_writer.writerow(
                    (e['frame_id'], self.g_pool.timestamps[e['frame_id']], e['srf_name'], e['srf_uid'], e['event']))
        ...
        for s in self.surfaces:
            ...
            with open(os.path.join(metrics_dir, 'srf_positons' + surface_name + '.csv'), 'w', encoding='utf-8',
                      newline='') as csvfile:
                csv_writer = csv.writer(csvfile, delimiter=',')
                ...
                for idx, ts, ref_srf_data in zip(range(len(self.g_pool.timestamps)), self.g_pool.timestamps, s.cache):
                    if in_mark <= idx < out_mark:
                        if ref_srf_data is not None and ref_srf_data is not False:
                            csv_writer.writerow((idx, ts, ref_srf_data['m_to_screen'], ref_srf_data['m_from_screen'],
                                                 ref_srf_data['detected_markers']))

            # save gaze on srf as csv.
            with open(os.path.join(metrics_dir, 'gaze_positions_on_surface' + surface_name + '.csv'), 'w',
            ...
            with open(os.path.join(metrics_dir, 'fixations_on_surface' + surface_name + '.csv'), 'w', encoding='utf-8',
                      newline='') as csvfile:
                csv_writer = csv.writer(csvfile, delimiter=',')
                ...
                for f_on_s in removed_duplicates:
                    ...
                    csv_writer.writerow((f['id'], f['timestamp'], f['duration'], f['start_frame_index'],
                                         f['end_frame_index'], f_x, f_y, f_x * s.real_world_size['x'],
    ...
    def cleanup(self):
        ...
        self.surface_definitions["offline_square_marker_surfaces"] = [rs.save_to_dict() for rs in self.surfaces if
                                                                      rs.defined]
